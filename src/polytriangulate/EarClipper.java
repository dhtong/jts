...
public class EarClipper
{
  ...
  /**
   * The shell coordinates are maintain in CW order. This means that for convex
...
//      if (isValidEarFast(iEar[0], iEar[1], iEar[2])) {
      if (isValidEarSlow(iEar[0], iEar[1], iEar[2])) {
        ...
        PolygonTriangle ear = new PolygonTriangle(iEar[0], iEar[1], iEar[2]);
        triList.add(ear);
   ...
   * Could be possibly replaced with checking whether
   * any other vertices lie inside the ear 
   * - which could be optimized with a spatial index
   * on the vertices. 
   * 
   * @param k0
   * @param k1
...
//      return false;
  ...
  {
    Coordinate[] triRing = new Coordinate[] { 
        polyShell.getCoordinate(k0),
        polyShell.getCoordinate(k1), 
        polyShell.getCoordinate(k2), 
        polyShell.getCoordinate(k0) };
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (v.equals2D(triRing[0])) continue;
      if (v.equals2D(triRing[1])) continue;
      if (v.equals2D(triRing[2])) continue;
      
      // not valid if vertex is contained in tri
      if (CGAlgorithms.isPointInRing(v, triRing)) return false;
    }
    return true;
  }
  ...
  private Polygon createPolygon(final PolygonTriangle t)
  {
    ...
    return gf.createPolygon(
        gf.createLinearRing(new Coordinate[] { polyShellCoords.get(vertices[0]),
            polyShellCoords.get(vertices[1]), polyShellCoords.get(vertices[2]),
   ...
   * the lowest upwards. As the resulting shell develops, a hole might be added
...
class HoleJoiner
{
  ...
  private void joinHoleToShell(Geometry hole)
  {
    ...
    /*
     * Note: it's important to scan the shell vertices in reverse so that if a
     * hole ends up being joined to what was originally another hole, the
     * previous hole's coordinates appear in the shell before the new hole's
     * coordinates (otherwise the triangulation algorithm tends to get stuck).
     */
